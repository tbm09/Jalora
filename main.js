...
(function () {
  ...
  (function initDesignSlider() {
    ...
    const labelEl = document.getElementById('selected-design-label');
    const selectBtn = document.getElementById('select-design-btn');
    const quoteDesign = document.getElementById('quote-design');
    ...
    function update() {
      ...
      dots.forEach(function (d, i) {
        d.classList.toggle('bg-jalora-600', i === index);
        d.classList.toggle('bg-jalora-200', i !== index);
      });
      const slide = slides[index];
      const name = slide.getAttribute('data-design') || '';
    ...
    dots.forEach(function (d) {
      d.addEventListener('click', function () { go(parseInt(d.dataset.index, 10)); });
    });
    ...
    if (selectBtn) {
      selectBtn.addEventListener('click', function () {
        ...
        var designName = slide.getAttribute('data-design') || '';
        var designId = slide.getAttribute('data-design-id') || '';
        if (quoteDesign) quoteDesign.value = designName + ' (Design #' + designId + ')';
        if (typeof goToQuote === 'function') goToQuote({});
      });
    }
  ...
  if (quoteForm) {
    quoteForm.addEventListener('submit', function (e) {
      ...
      const lines = [
        ...
        `Selected Design: ${(fd.get('design') || '').trim() || 'Not specified'}`,
